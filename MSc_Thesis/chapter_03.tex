...
\begin{figure*}[h!]
    ...
    \end{subfigure}
    \caption[]
    {\small \mgg distributions before (left) and after (right) the common preselection.}
    \label{diphotondist}
\end{figure*}
...
\begin{table}[h!]
    ...
    Sub-leading Jet btag & btag discriminator score of the sub-leading good jet \\
...
\begin{figure*}[h!]
    ...
    \begin{subfigure}[b]{0.475\textwidth}
        \centering
        \includegraphics[width=\textwidth]{DNN_Score_WW_logy.pdf}
        \vspace{-0.5cm}
        \firstsubcaption{DNN score}
        \label{GGFH}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.475\textwidth}  
        \centering 
        \includegraphics[width=\textwidth]{WWgg_1L_prefit_log_unblinded_False_HL.pdf}
        \vspace{-0.5cm}
        \firstsubcaption{Di-photon mass}  
        \label{VBFH}
    \end{subfigure}
    \vskip\baselineskip
    \begin{subfigure}[b]{0.475\textwidth}   
    ...
    \begin{subfigure}[b]{0.475\textwidth}   
        ...
        \label{6}
    \end{subfigure}
    \caption{\small DNN score (a), di-photon invariant mass (b) and some of the DNN input distributions.} 
    \label{hasOneL_plots}
\end{figure*}

\begin{table}[h!]
    \centering
    \caption{One Lepton final state DNN score categories.}
    \begin{tabular}{ c c }
    \hline
    Category & Definition \\
    \hline
...
A cut-based analysis is performed in this final state. The events are required to have at least two oppositely charged good leptons ($e^+e^-$, $\mu^+\mu^-$, $e^\pm\mu^\mp$). These two leptons are separated from each other by an angular distance of at least $0.4$. The lepton pair that have invariant mass value between $80-100$ GeV, and the invariant mass of the leading electron and the leading photon in 5 GeV vicinity of Z boson mass in the events that have at least one electron are removed in order to suppress the Z boson contribution (Z veto). The full list of requirements in this final state is shown in \autoref{hasTwoL_cuts}. The di-photon invariant mass distribution in this final state is shown in \autoref{hasTwoL_InvM}.

\begin{table}[!h]
    \centering
        \caption{Requirements of the Two Lepton Channel.}
        \begin{tabular}{c|c}
            Variable & Selection \\ \hline
            $\Delta R(l,l)$ & $> 0.4$ \\
            \pt of the leading lepton & $> 20$ GeV\\
            \pt of sub-leading lepton & $> 10$ GeV \\
            $E_T^{miss}$ & $> 20 $ GeV \\
...
\begin{figure}[h!]
	\includegraphics[scale=0.7]{replace.png}
	\vspace{2mm}
	\caption{}
	\label{hasTwoL_InvM}
\end{figure}
