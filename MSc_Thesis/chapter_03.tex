...
The DNNs are trained using skimmed signal and background samples and are evaluated on the full samples in order to perform categorisation. The feature variables used as input to train the DNN include the kinematic variables such as \pt, $\eta$, $\phi$ and energy values of objects along with the re-weighted variables. The full list of input features is given in \autoref{dnninputs}. During the training, events in the signal MC samples are given a target value of 1 and events in the background MC samples are given a target value of 0. The resulting DNN score distribution along with some of the input distribution and ROC curves are shown in \autoref{hasOneL_plots}. The events are then categorised into four making used of the DNN score distribution such that the expected significance is maximised, as shown in \autoref{hasOneL_dnncategories}. The DNN hyper-parameter settings in this final state is given in \auttoref{dnnpars1}. A cut-flow table, showing the number of events in the semi-leptonic channel and its DNN score categories is given in \autoref{semileptonic_cutflow} along with the number of events before any selection.
...
\textbf{1$\tau$ Final State of $HH\rightarrow{\tau\tau\gamma\gamma}$}

In this final state, two multi-class DNNs are trained following the same strategy as in semi-leptonic final state of the \wwgg leg of the decay. The input variables are shown in \autoref{dnninputs_1tau}. The information of $\tau$ leptons are added to the training while removing the electron, muon and some jet information in this final state. Full distributions are given in \autoref{dnninputDists_tau} along with the training weights and the ROC curves. The resulting categorisation is shown in \autoref{onetau-cats}.
