...
The python code that is used to define object, event selection and categorisations is given in Appendix \autoref{bamboocode}.
...
The uncertainty on the experiments has two components; \textbf{statistical} uncertainty and \textbf{systematical} uncertainty. The former depends on the amount of data collected while the latter arises from the inaccuracy of the experimental setup due to detector, or from theoretecal calculations. The experiment-induced systematic uncertainties are caues by the insufficient knowledge of the detector response, and the theoretical systematic uncertaities affect the modelling of signal and background processes. These systematical uncertainties are taken as nuisance parameters in the statistical analysis such as normalisation uncertainty which affect the signal/background yield, and shape uncertainties which affect the distribution of the observables.The experimental systematic uncertainties have several sources;
\begin{itemize}
    ...
    \item Jet energy scale which is computed by propagating the variation of the jet energy thorugh the event reconstruction
\end{itemize}
...
The theoretical uncertainties comes from the choice of the PDF set, the uncertainty on the $_alpha_S$, and the renormalisation and factorisation QCD scale. These uncertainties affect both the signal and background processes. Additionally, an uncertainty related to missing top quark mass effects has a contribution for the double Higgs signal process. A summary of the theoretical uncertainties is given in \autoref{ThUncertainties}.
